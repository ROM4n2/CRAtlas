...
完整开发流程请参阅：`docs/superpowers/development-path.md`

核心要点：
- 每个 Task 开发前写 brief（`.superpowers/sdd/phase{N}-task-{M}-brief.md`）
- 每个 Task 开发后写 report（`.superpowers/sdd/phase{N}-task-{M}-report.md`）
- 使用 subagent-driven-development 技能：implementer + reviewer 双代理制衡
- 使用标准 prompt 模板派发子代理（implementer-prompt.md / task-reviewer-prompt.md）
- 每个 Task 完成后更新 progress.md 台账
- 全部完成后 whole-branch final review + finishing-a-development-branch

## 关键文件

- 开发路径：docs/superpowers/development-path.md
